...
def processUploadFiles(filename,filesize,files,update,bot,message,thread=None,jdb=None):
    try:
        bot.editMessageText(message,'Analizando conectividad a la nube☁...')
        bot.editMessageText(message,'Analizando velocidad de subida☁...')
        bot.editMessageText(message,'Analizando tamaño del archivo☁...')
        ...
        if cloudtype == 'moodle':
                ...
                for f in files:
                    ...
                    while resp is None:
                          ...
                          if user_info['uploadtype'] == 'draft':
                             fileid,resp = client.upload_file_draft(f,progressfunc=uploadFile,args=(bot,message,originalfile,thread))
            ...
            if loged:
               ...
               filesdata = []
               for f in files:
                   data = client.upload_file(f,path=remotepath,progressfunc=uploadFile,args=(bot,message,originalfile,thread))
...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if file_size > max_file_size:
        ...
        file_upload_count = 1
    bot.editMessageText(message,'📄Extrayendo link📄...')
    evidname = ''
    files = []
    if client:
...
def ddl(update,bot,message,url,file_name='',thread=None,jdb=None):
    downloader = Downloader('Descargas/')
...
def megadl(update,bot,message,megaurl,thread=None,jdb=None):
    ...
    try:
        ...
        if '/add' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    bot.sendMessage(update.message.chat.id,'❌Error en el comando /add username❌')
            else:
                bot.sendMessage(update.message.chat.id,'❌No tiene permiso para usar este comando❌')
            return
        if '/ban' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
                try:
                    user = str(msgText).split(' ')[1]
                    if user == username:
                        bot.sendMessage(update.message.chat.id,'❌No puede banearse usted mismo❌')
                        return
                    jdb.remove(user)
                    jdb.save()
                    ...
                    bot.sendMessage(update.message.chat.id,'❌Error en el comando /ban username❌')
            else:
                bot.sendMessage(update.message.chat.id,'❌No tiene permiso para usar este comando❌')
            return
        if '/data' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
                bot.sendMessage(update.message.chat.id,'Base De Datos de MoodleBot Down/Up')
        ...
        if '/uptype' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'❌Error en el comando /uptype (tipo de subida (evidence,draft,blog))❌')
            return
        if '/proxy' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/start' in msgText:
            start_msg = '🤖Bot: SpeedUploaderPro5\n'
            start_msg+= '🎩Desarrollador: @Jose_753\n'
            start_msg+= '🟢Estado: Listo para operar\n'
            start_msg+= '🛠Mejoras: Subida a las nubes más rapida, use /tutorial para aprender a usar el bot\n'
            ...
            #    bot_token = '5210107354:AAGlRlQ50XaOBEc2EDsL251enPFTV2RZCqA'
